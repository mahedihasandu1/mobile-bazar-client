...
const MyOrders = () => {
    ...
    const { data: bookings = [], isLoading, refetch } = useQuery({
        queryKey: ['bookings'],
        queryFn: async () => {
            const res = await fetch(`https://mobile-bazar-server-mahedihasandu1.vercel.app/bookedProduct?userEmail=${user?.email}`)
            const data = res.json()
            return data
        }
    })
    ...
    const handleDelete = (id) => {
        ...
        if (proceed) {
            fetch(`https://mobile-bazar-server-mahedihasandu1.vercel.app/bookedProduct?id=${id}`, {
                method: 'DELETE',
                headers: {
