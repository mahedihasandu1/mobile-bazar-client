...
const ReportedItems = () => {
    const { data: items = [], refetch, isLoading } = useQuery({
        queryKey: ['items'],
        queryFn: async () => {
            const res = await fetch('https://mobile-bazar-server-mahedihasandu1.vercel.app/reportItem')
            const data = res.json();
            return data;
        }
    })
    const handleDeleteProduct = (item) => {
        ...
        if (proceed) {
            fetch(`https://mobile-bazar-server-mahedihasandu1.vercel.app/reportItem?id=${item._id}`, {
                method: 'DELETE',
                headers: {
                ...
                .then(data => {
                    if (data.deletedCount > 0) {
                        fetch(`https://mobile-bazar-server-mahedihasandu1.vercel.app/adsProducts?id=${item.productId}`, {
                            method: 'DELETE',
                            headers: {
                                authorization: `bearer ${localStorage.getItem('accessToken')}`
                            }
                        ...
                        fetch(`https://mobile-bazar-server-mahedihasandu1.vercel.app/products/${item.productId}`, {
                            method: 'DELETE',
                            headers: {
                                authorization: `bearer ${localStorage.getItem('accessToken')}`
                            }
